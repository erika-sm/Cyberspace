...
const addNewUser = async (req, res) => {
  ...
  const user = findUsername(res.locals.users, newUser.name);

  if (!user) {
    res.locals.users.push(newUser);

    return sendResponse(res, 200, newUser, "Registration successful");
  } else
    return sendResponse(
      res,
      400,
      newUser,
      "Name already exists in our database. Please try again"
    );
};
...
const updateUser = (req, res) => {
  ...
  const { id, name, bio, friends, avatarUrl } = updatedUser;
  ...
  if ((!id, !name || !bio || !avatarUrl || !friends))
    return sendResponse(
      res,
      400,
      updatedUser,
  ...
  res.locals.users[index] = { id, name, bio, friends, avatarUrl };
